...
def test_get_por_referencia_sucesso(jwt_authenticated_client_sme, flag_paa, periodo_paa_1):
    response = jwt_authenticated_client_sme.get(f'/api/periodos-paa/?referencia=04/2025')
    content = json.loads(response.content)
...
def test_get_por_referencia_sem_resultado(jwt_authenticated_client_sme, flag_paa, periodo_paa_1):
    response = jwt_authenticated_client_sme.get(f'/api/periodos-paa/?referencia=04/2026')
    content = json.loads(response.content)
...
def test_get_404(jwt_authenticated_client_sme, flag_paa, periodo_paa_1):
    response = jwt_authenticated_client_sme.get(f'/api/periodos-paa/b737979c-66e2-4d38-b266-652aa1f0fe5d/')
