...
def test_get_sucesso_filtro_associacao(jwt_authenticated_client_sme, flag_paa, paa):
    response = jwt_authenticated_client_sme.get(f'/api/paa/?associacao_uuid={paa.associacao.uuid}')
    content = json.loads(response.content)
...
def test_get_404(jwt_authenticated_client_sme, flag_paa, paa):
    response = jwt_authenticated_client_sme.get('/api/periodos-paa/c4ce47ec-4ce2-4c34-e2c4-6e24a144fe4e/')
