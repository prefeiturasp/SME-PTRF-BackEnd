...
def test_get_previa_atividades_previstas(jwt_authenticated_client_sme, flag_paa, atividade_estatutaria_paa_factory, recurso_proprio_paa_factory):

    atividade = atividade_estatutaria_paa_factory()

    paa = atividade.paa

    recurso_proprio_paa_factory(paa=paa)

    response = jwt_authenticated_client_sme.get(
        f"/api/paa/{str(paa.uuid)}/previa-atividades-previstas/")

    result = response.json()

    assert response.status_code == status.HTTP_200_OK
    assert "atividades_estatutarias" in result
    assert "recursos_proprios" in result
    assert "total_recursos_proprios" in result
    assert result["total_recursos_proprios"] == result["recursos_proprios"][0]["valor"]
