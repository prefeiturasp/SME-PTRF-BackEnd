...
from sme_ptrf_apps.paa.tasks.gerar_documento_paa_retificacao import (
    gerar_documento_paa_retificacao_async,
    MAX_RETRIES,
)

pytestmark = pytest.mark.django_db

_SERVICE_PATH = 'sme_ptrf_apps.paa.tasks.gerar_documento_paa_retificacao.DocumentoPaaService'
_GERAR_PDF_PATH = 'sme_ptrf_apps.paa.tasks.gerar_documento_paa_retificacao.gerar_arquivo_documento_paa_pdf'
_RETIFICACAO_SERVICE_PATH = 'sme_ptrf_apps.paa.tasks.gerar_documento_paa_retificacao.RetificacaoPaaService'


def test_gerar_documento_retificacao_async_sucesso(paa, usuario_task):
    ...
    with patch(_SERVICE_PATH) as mock_service_class, \
        ...
        result = gerar_documento_paa_retificacao_async.apply(
...
def test_gerar_documento_retificacao_async_service_criado_com_retificacao_true(paa, usuario_task):
    """Testa que DocumentoPaaService é instanciado com retificacao=True e previa=False."""
    with patch(_SERVICE_PATH) as mock_service_class, \
         patch(_GERAR_PDF_PATH), \
         patch(_RETIFICACAO_SERVICE_PATH) as mock_retificacao_class:
        ...
        gerar_documento_paa_retificacao_async.apply(
            args=[str(paa.uuid), usuario_task.username]
        )

    mock_service_class.assert_called_once_with(
        paa=paa, usuario=usuario_task.username, previa=False, logger=ANY, retificacao=True
    )


def test_gerar_documento_retificacao_async_gerar_pdf_chamado_com_alteracoes(paa, usuario_task):
    """Testa que gerar_arquivo_documento_paa_pdf é chamado com alteracoes e previa=False."""
...
def test_gerar_documento_retificacao_async_falha_sem_alteracoes(paa, usuario_task):
...
def test_gerar_documento_retificacao_async_falha_gerar_pdf_chama_marcar_erro(paa, usuario_task):
...
def test_gerar_documento_retificacao_async_falha_max_retries_excedido(paa, usuario_task):
...
def test_gerar_documento_retificacao_async_falha_quando_paa_nao_encontrado():
...
def test_gerar_documento_retificacao_async_marcar_erro_nao_chamado_quando_documento_paa_none(paa, usuario_task):
    """Testa que marcar_erro() não é chamado quando documento_paa é None (falha antes do iniciar)."""
    with patch(_SERVICE_PATH) as mock_service_class, \
         patch(_RETIFICACAO_SERVICE_PATH):

        mock_service_class.return_value.preparar_documento_para_task.side_effect = Exception("Falha no iniciar")
...
def test_gerar_documento_retificacao_async_retificacao_service_recebe_usuario_objeto(paa, usuario_task):
...
def test_gerar_documento_retificacao_async_falha_quando_usuario_nao_encontrado(paa):
...
def test_gerar_documento_retificacao_async_gerar_pdf_nao_chamado_sem_alteracoes(paa, usuario_task):
...
def test_gerar_documento_retificacao_async_marcar_erro_nao_chamado_quando_paa_nao_encontrado():
