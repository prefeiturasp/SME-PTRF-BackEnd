...
@pytest.mark.django_db
def test_obter_total_recurso_proprio_paa(
        jwt_authenticated_client_sme, flag_paa, recurso_proprio_paa_factory, paa, fonte_recurso_paa, associacao):
    recurso_proprio_paa_factory.create(
        descricao="ABC 004", valor=100.00, paa=paa, fonte_recurso=fonte_recurso_paa, associacao=associacao)
    recurso_proprio_paa_factory.create(
        descricao="ABC 005", valor=200.00, paa=paa, fonte_recurso=fonte_recurso_paa, associacao=associacao)
    recurso_proprio_paa_factory.create(
        descricao="ABC 006", valor=300.00, paa=paa, fonte_recurso=fonte_recurso_paa, associacao=associacao)
    recurso_proprio_paa_factory.create(
        descricao="ABC 007", valor=150.00, paa=paa, fonte_recurso=fonte_recurso_paa, associacao=associacao)
...
@pytest.mark.django_db
def test_filtra_recursos_proprios_por_paa_uuid(
    ...
    recurso_proprio_paa_factory.create(
        descricao="Recurso PAA 1", valor=100.00, paa=paa1, fonte_recurso=fonte_recurso_paa, associacao=associacao)
    recurso_proprio_paa_factory.create(
        descricao="Recurso PAA 2", valor=200.00, paa=paa2, fonte_recurso=fonte_recurso_paa, associacao=associacao)
...
@pytest.mark.django_db
def test_filtra_total_recursos_proprios_por_paa_uuid(
        jwt_authenticated_client_sme,
        flag_paa,
        recurso_proprio_paa_factory,
        paa_factory,
