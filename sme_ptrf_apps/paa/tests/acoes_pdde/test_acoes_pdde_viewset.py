...
def test_destroy_acao_pdde_com_receitas_vinculadas(jwt_authenticated_client_sme, flag_paa, documento_paa_factory):
    ...
    paa_vigente = PaaFactory(periodo_paa=periodo_vigente, status=PaaStatusEnum.EM_ELABORACAO.name)

    # Criar documento PAA
    documento_paa_factory(paa=paa_vigente, versao="FINAL", status_geracao="CONCLUIDO")
    ...
    assert (
        ...
        "um Plano Anual de Atividades (PAA).") in response.data["detail"]
...
def test_destroy_acao_pdde_sem_receitas_vinculadas(jwt_authenticated_client_sme, flag_paa):
    ...
    # Criar PAA para o período vigente
    PaaFactory(periodo_paa=periodo_vigente)
