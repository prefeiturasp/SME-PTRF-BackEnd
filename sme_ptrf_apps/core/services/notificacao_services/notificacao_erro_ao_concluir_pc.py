...
def notificar_erro_ao_concluir_pc(prestacao_de_contas, usuario, associacao, periodo, enviar_email=False):
    ...
    if usuario:
        ...
        descricao_mensagem = (
            f"Ocorreu um erro ao concluir a PC do período {periodo.referencia} da associação {associacao}, "
            "tente novamente\n\n"
    ...
    logger.info('Finalizando a geração de notificação de erro ao concluir PC')
