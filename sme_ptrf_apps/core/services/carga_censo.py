...
from ..models import Associacao, Unidade, Censo, Arquivo
...
class ProcessaCenso:
    ...
    @classmethod
    def processa_censo(
        cls, reader: Iterator[List[str]], arquivo: Arquivo
    ) -> Tuple[int, int]:
        ...
        for lin, row in enumerate(reader):
            ...
            censo = Censo.objects.create(
                unidade=unidade, quantidade_alunos=quantidade_alunos, ano=ano
            )
        ...
        logger.info(
            f"Importadas {importadas} dados de censo. Erro na importação de {erros} dados do censo."
        )
...
def carrega_censo(arquivo: Arquivo) -> None:
