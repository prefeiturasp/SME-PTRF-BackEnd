...
def gerar_notificacao_inicio_periodo_prestacao_de_contas_async():
    ...
    logger.info('Iniciando a geração de notificação início período prestação de contas async')

    notificar_inicio_periodo_prestacao_de_contas()

    logger.info('Finalizando a geração de notificação início período prestação de contas async')
