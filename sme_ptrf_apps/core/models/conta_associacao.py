...
class ContasComSolicitacaoDeEncerramento(models.Manager):
    def get_queryset(self):
        return (
            super(ContasComSolicitacaoDeEncerramento, self)
            .get_queryset()
            .filter(solicitacao_encerramento__isnull=False)
        )
...
class ContaAssociacao(ModeloBase):
    ...
    tipo_conta = models.ForeignKey(
        "TipoConta", on_delete=models.PROTECT, blank=True, null=True
    )
    status = models.CharField(
        "status", max_length=15, choices=STATUS_CHOICES, default=STATUS_ATIVA
    )
    banco_nome = models.CharField(
        "Nome do banco", max_length=50, blank=True, default=""
    )
    ...
    numero_cartao = models.CharField(
        "Nº do cartão", max_length=80, blank=True, default=""
    )
    data_inicio = models.DateField(
        verbose_name="Data de início da conta", blank=True, null=True
    )
    ...
    def __str__(self):
        ...
        tipo_conta = (
            self.tipo_conta.nome if self.tipo_conta else "Tipo de conta indefinido"
        )
    ...
    def pode_encerrar(self, data_encerramento):
        from sme_ptrf_apps.core.services.encerramento_conta_associacao_service import (
            ValidaDataDeEncerramento,
        )
    ...
    def todos_valores_reprogramados_foram_preenchidos(self, valida_valor_dre=False):
        ...
        for acao_associacao in self.associacao.acoes.exclude(
            ...
            if acao_associacao.acao.aceita_custeio:
                ...
                if valor_reprogramado_por_aplicacao.exists():
                    lista_de_valores_reprogramados.append(
                        valor_reprogramado_por_aplicacao.first()
                    )
        ...
        if valida_valor_dre:
            valores_reprogramados_preenchidos = valores_reprogramados.exclude(
                valor_ue=None
            ).exclude(valor_dre=None)
        else:
            valores_reprogramados_preenchidos = valores_reprogramados.exclude(
                valor_ue=None
            )
    ...
    def valida_status_valores_reprogramados(self):
        ...
        status_conferencia_dre = (
            Associacao.STATUS_VALORES_REPROGRAMADOS_EM_CONFERENCIA_DRE
        )
        ...
        if status_atual == status_nao_finalizado:
            if not self.valida_se_algum_valor_reprogramado_foi_preenchido():
                ...
                if not self.todos_valores_reprogramados_foram_preenchidos():
                    ...
                    resultado[
                        "mensagem"
                    ] = "Existem valores reprogramados preenchidos, mas não concluídos. É necessário finalizar o preenchimento dos Valores reprogramados para solicitar o encerramento da conta bancária."
                else:
                    resultado["pode_encerrar_conta"] = True
                    resultado["mensagem"] = None

        elif (
            status_atual == status_correcao_ue or status_atual == status_conferencia_dre
        ):
            if self.todos_valores_reprogramados_foram_preenchidos(
                valida_valor_dre=True
            ):
                resultado["pode_encerrar_conta"] = True
                resultado[
                    "mensagem"
                ] = "O pedido de solicitação de encerramento de conta bancária foi efetuado com sucesso. Existem valores reprogramados concluídos e o encerramento definitivo da conta será realizado após a geração da PC e a conclusão da análise pela DRE."
            else:
                resultado["pode_encerrar_conta"] = False
                resultado[
                    "mensagem"
            ...
            if self.todos_valores_reprogramados_foram_preenchidos(
                ...
                resultado[
                    "mensagem"
                ] = "O pedido de solicitação de encerramento de conta bancária foi efetuado com sucesso. O encerramento definitivo da conta será realizado após a geração da PC e a conclusão da análise pela DRE."
