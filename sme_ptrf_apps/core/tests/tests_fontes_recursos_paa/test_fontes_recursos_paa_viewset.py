...
def test_lista_fontes_recursos_paa(jwt_authenticated_client_sme, flag_paa):
    ...
    assert response.data[0]["nome"] == "Campanhas com arrecadação"


@pytest.mark.django_db
def test_obtem_fonte_recurso_paa(jwt_authenticated_client_sme, flag_paa):
    fonte_recurso = FonteRecursoPaa.objects.first()
    response = jwt_authenticated_client_sme.get(f"/api/fontes-recursos-paa/{fonte_recurso.uuid}/")

    assert response.status_code == status.HTTP_200_OK
    assert response.data["nome"] == "Campanhas com arrecadação"
