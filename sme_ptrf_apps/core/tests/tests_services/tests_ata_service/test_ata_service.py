...
@pytest.mark.django_db(transaction=False)
def test_validar_campos_invalidos(ata_2022_test_campos_invalidos, presente_ata_membro_arnaldo,
                                  presente_ata_membro_falcao):
...
@pytest.mark.django_db
def test_verifica_precisa_professor_gremio_ata_com_tipo_configurado_sem_despesas(ata_prestacao_conta_iniciada,
                                                                                 parametros):
...
def test_verifica_precisa_professor_gremio_ata_com_despesas_completas_gremio_no_periodo(
    ...
    """Testa se verifica_precisa_professor_gremio_ata retorna True quando
        há despesas completas com ação grêmio no período"""
    tipo_unidade = ata_prestacao_conta_iniciada.associacao.unidade.tipo_unidade
    parametros.tipos_unidades_professor_gremio = [tipo_unidade]
    parametros.save()
