...
def test_list_contas_associacoes_filtro_status_sucesso(
    ...
    response = jwt_authenticated_client_a.get(
        f'/api/contas-associacoes/?status={conta_associacao_2[0].status}',
        content_type='application/json')
    result = json.loads(response.content)

    assert response.status_code == status.HTTP_200_OK
    assert len(result["results"]) == 2
    assert result["results"][0]["uuid"] == str(conta_associacao_2[0].uuid)
    assert result["results"][0]["tipo_conta_dados"]["uuid"] == str(conta_associacao_2[0].tipo_conta.uuid)
    assert result["results"][0]["associacao_dados"]["uuid"] == str(conta_associacao_2[0].associacao.uuid)
    assert result["results"][1]["uuid"] == str(conta_associacao_2[1].uuid)
    assert result["results"][1]["tipo_conta_dados"]["uuid"] == str(conta_associacao_2[1].tipo_conta.uuid)
    assert result["results"][1]["associacao_dados"]["uuid"] == str(conta_associacao_2[1].associacao.uuid)
...
def test_list_contas_associacoes_filtro_status_nao_encontrado(
    ...
    response = jwt_authenticated_client_a.get(
        f'/api/contas-associacoes/?status=nome_aleatorio',
        content_type='application/json')
    result = json.loads(response.content)
...
def test_list_contas_associacoes_filtro_nome_associacao_e_status_sucesso(
    ...
    response = jwt_authenticated_client_a.get(
        f'/api/contas-associacoes/?associacao_nome={conta_associacao_2[0].associacao.nome}&status={conta_associacao_2[0].status}',
        content_type='application/json')
    result = json.loads(response.content)
...
def test_list_contas_associacoes_filtro_nome_associacao_e_status_nao_encontrado(
    ...
    response = jwt_authenticated_client_a.get(
        f'/api/contas-associacoes/?associacao_nome={conta_associacao_2[0].associacao.nome}&status=status_aleatorio',
        content_type='application/json')
    result = json.loads(response.content)
...
def test_list_contas_associacoes_filtro_todos_sucesso(
    ...
    response = jwt_authenticated_client_a.get(
        f'/api/contas-associacoes/?associacao_nome={conta_associacao_2[0].associacao.nome}&tipo_conta_nome={conta_associacao_2[0].tipo_conta.nome}&status={conta_associacao_2[0].status}',
        content_type='application/json')
    result = json.loads(response.content)
...
def test_list_contas_associacoes_filtro_todos_nao_encontrado(
    ...
    response = jwt_authenticated_client_a.get(
        f'/api/contas-associacoes/?associacao_nome={conta_associacao_2[0].associacao.nome}&tipo_conta_uuid=c636046a-8d67-49a0-9a4c-0666af48ea50&status={conta_associacao_2[0].status}',
        content_type='application/json')
    result = json.loads(response.content)
