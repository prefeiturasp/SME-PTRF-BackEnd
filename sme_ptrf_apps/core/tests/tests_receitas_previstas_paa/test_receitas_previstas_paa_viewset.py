...
from sme_ptrf_apps.core.fixtures.factories import ReceitaPrevistaPaaFactory, FlagFactory
...
@pytest.mark.django_db
def test_cria_receita_prevista_paa(jwt_authenticated_client_sme, acao_associacao, flag_paa):
    data = {"previsao_valor_custeio": 1234, "acao_associacao": acao_associacao.id}
...
@pytest.mark.django_db
def test_cria_receita_prevista_paa_sem_acao_associacao(jwt_authenticated_client_sme, flag_paa):
    data = {"previsao_valor_custeio": 2000 }
    response = jwt_authenticated_client_sme.post("/api/receitas-previstas-paa/", data)

    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert 'acao_associacao' in response.data
    assert response.data["acao_associacao"] == ["O campo Ação de Associação é obrigatório."]
