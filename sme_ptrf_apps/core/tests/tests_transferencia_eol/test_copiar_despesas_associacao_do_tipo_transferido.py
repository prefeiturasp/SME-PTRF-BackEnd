...
def test_nao_deve_copiar_as_despesas_e_reateios_da_associacao_origem_caso_comportamento_nao_seja_transferir_tipo_conta(
    ...
    transferencia_eol_copiar_todas_contas.copiar_despesas_associacao_do_tipo_transferido(transf_eol_associacao_eol_transferido, transf_eol_associacao_nova)

    # Nenhum dos gastos e rateios da associação original devem ser copiados para a nova associação
    despesas_original = transf_eol_associacao_eol_transferido.despesas.all()
    despesas_nova = transf_eol_associacao_nova.despesas.all()

    assert despesas_nova.count() == 0, "Não deve ter copiado nenhuma despesa"
    assert despesas_original.count() == 2, "A associação original deve manter as despesas originais"
