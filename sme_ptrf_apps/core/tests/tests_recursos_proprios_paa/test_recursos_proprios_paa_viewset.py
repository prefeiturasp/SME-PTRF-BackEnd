...
def test_obter_recurso_proprio_paa_404(jwt_authenticated_client_sme, flag_paa, recurso_proprio_paa_factory):
    ...
    response = jwt_authenticated_client_sme.get(f"/api/recursos-proprios-paa/a41efdd4-068b-43a8-9211-9339bceda4d8/")
...
@pytest.mark.django_db
def test_cria_recurso_proprio_paa_campos_faltando(jwt_authenticated_client_sme, flag_paa, associacao, fonte_recurso_paa):
    data = {
        "fonte_recurso": str(fonte_recurso_paa.uuid),
        "associacao": str(associacao.uuid),
        "data_prevista": "2024-04-04",
...
def test_atualiza_recurso_proprio_paa_404(jwt_authenticated_client_sme, flag_paa, recurso_proprio_paa):
    ...
    response = jwt_authenticated_client_sme.patch(f"/api/recursos-proprios-paa/a41efdd4-068b-43a8-9211-9339bceda4d8/", data)
...
def test_obter_total_recurso_proprio_paa(jwt_authenticated_client_sme, flag_paa, recurso_proprio_paa_factory):
    ...
    response = jwt_authenticated_client_sme.get(f"/api/recursos-proprios-paa/total/")
    assert response.status_code == status.HTTP_200_OK
