...
def test_deve_notificar_usuarios(periodo_notifica_pendencia_envio_pc, usuario_notificavel, associacao_a):
    ...
    assert notificacao.descricao == (
        f"Terminou o período de prestações de contas para a associação {associacao_a.unidade.codigo_eol} - "
        f"{associacao_a.unidade.nome} e você ainda não enviou sua PC.\n\n"
