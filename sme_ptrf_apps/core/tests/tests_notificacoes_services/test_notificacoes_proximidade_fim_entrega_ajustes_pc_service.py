...
def test_deve_notificar_usuarios_5_dias_antes(
    ...
    periodo = devolucao_notifica_proximidade_fim_entrega_ajustes_pc.prestacao_conta.periodo
    ...
    assert notificacao.titulo == f"O prazo para envio dos ajustes da PC está se aproximando {periodo.referencia} | Prazo: 5 dia(s)"
    assert notificacao.descricao == (
        f"Faltam apenas 5 dia(s) para o fim do prazo de envio dos ajustes de sua prestações de "
        f"contas de {periodo.referencia}. Fique atento para não perder o prazo e realize os ajustes solicitados.\n\n"
