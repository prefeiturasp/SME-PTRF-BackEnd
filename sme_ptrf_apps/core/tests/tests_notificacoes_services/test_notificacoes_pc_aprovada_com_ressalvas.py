...
from ....core.services.notificacao_services import notificar_prestacao_de_contas_aprovada_com_ressalvas

pytestmark = pytest.mark.django_db


def test_deve_notificar_usuarios(prestacao_notifica_pc_aprovada_com_ressalvas, motivo_aprovacao_ressalva_x, usuario_notificavel, associacao_a):
    assert not Notificacao.objects.exists()
    motivos = f" {motivo_aprovacao_ressalva_x.motivo} \n"
    outros_motivos = 'Esse é o outro motivo de aprovação com ressalvas'
    notificar_prestacao_de_contas_aprovada_com_ressalvas(prestacao_notifica_pc_aprovada_com_ressalvas, motivo_aprovacao_ressalva_x.motivo, outros_motivos, enviar_email=False)
...
def test_nao_deve_notificar_usuarios_sem_permissao(prestacao_notifica_pc_aprovada_com_ressalvas, motivo_aprovacao_ressalva_x, usuario_nao_notificavel, associacao_a):
    assert not Notificacao.objects.exists()
    outros_motivos = 'Esse é o outro motivo de aprovação com ressalvas'
