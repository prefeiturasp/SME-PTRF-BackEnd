...
def test_notifica_pendencia_geracao_ata_apresentacao(visao_factory, grupo_acesso_factory, unidade_factory,
                                                     usuario_factory, periodo_factory, associacao_factory,
                                                     prestacao_conta_factory, periodo_inicial_associacao_factory):
    ...
    periodo_inicial_associacao_factory.create(periodo_inicial=periodo_2023_1, associacao=associacao)
    ...
    assert notificacao.descricao == (
        f"A ata de apresentação da PC {pc.periodo.referencia} não foi gerada e a DRE {pc.associacao.unidade.formata_nome_dre()} não pode receber a PC. Favor efetuar a geração da ata.\n\n"
        f"Recurso: {recurso.nome}\n"
    )
    assert notificacao.usuario == usuario_a_ser_notificado


def test_notifica_pendencia_geracao_ata_retificacao(visao_factory, grupo_acesso_factory, unidade_factory,
                                                    usuario_factory, periodo_factory, associacao_factory,
                                                    prestacao_conta_factory, periodo_inicial_associacao_factory):
    visao_ue = visao_factory.create(nome='UE')
    grupo_ue_1 = grupo_acesso_factory.create(name="ue_nivel1")
    ...
    assert notificacao.descricao == (
        f"A ata de retificação da PC {pc.periodo.referencia} não foi gerada e a DRE {pc.associacao.unidade.formata_nome_dre()} não pode receber a PC apresentada após acertos. Favor efetuar a geração da ata.\n\n"
        f"Recurso: {recurso.nome}\n"
    )
    assert notificacao.usuario == usuario_a_ser_notificado
