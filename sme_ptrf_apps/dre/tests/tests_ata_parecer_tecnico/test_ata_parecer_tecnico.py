...
def test_ata_retorna_presentes_por_recurso(
    ...
    response = jwt_authenticated_client_dre.get(
        f"/api/ata-parecer-tecnico/{str(ata.uuid)}/",
    ...
    presentes_na_ata = response.json()['presentes_na_ata']
...
def test_ata_retorna_lista_vazia_quando_nao_ha_presentes_do_recurso(
    jwt_authenticated_client_dre,
    recurso_factory,
):
    dre = DreFactory()
    ...
    response = jwt_authenticated_client_dre.get(
        f"/api/ata-parecer-tecnico/{ata.uuid}/",
        content_type="application/json",
        HTTP_X_RECURSO_SELECIONADO=str(recurso_ptrf.uuid),
    )
