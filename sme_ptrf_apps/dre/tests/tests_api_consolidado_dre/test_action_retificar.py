...
def test_retificar_consolidado_dre(
    ...
    payload = {
        'pcs_a_retificar': [prestacao_conta_x.uuid],
...
def test_retificar_consolidado_dre_sem_informar_pcs_deve_retornar_400(
    ...
    assert response.json() == {'pcs_a_retificar': ['É necessário informar ao menos uma PC para retificar.']}
...
def test_retificar_consolidado_dre_sem_informar_motivo_deve_retornar_400(
    ...
    payload = {
        'pcs_a_retificar': [f'{prestacao_conta_x.uuid}'],
        'motivo_retificacao': ''
    }
    ...
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.json() == {'motivo_retificacao': ['This field may not be blank.']}
