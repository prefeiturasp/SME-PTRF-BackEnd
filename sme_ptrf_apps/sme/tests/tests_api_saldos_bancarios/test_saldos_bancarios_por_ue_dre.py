...
def test_saldo_bancario_por_ue_dre(jwt_authenticated_client_sme, periodo_factory, associacao_factory, conta_associacao_factory, observacao_conciliacao_factory, unidade_factory):
    ...
    print(Unidade.objects.filter(tipo_unidade="DRE").count())
    ...
    dre_encontrada = next((obj for obj in result if obj["nome_dre"] == dre_saldos_bancarios.nome.replace("DIRETORIA REGIONAL DE EDUCACAO", "").strip()), None)
    
    assert response.status_code == status.HTTP_200_OK
    for associacao_data in dre_encontrada['associacoes']:
        associacao = associacao_data['associacao']
        assert associacao_data['saldo_total'] == associacoes_dict.get(associacao, None), f"saldo_total for {associacao} is not {associacoes_dict.get(associacao, None)}"
