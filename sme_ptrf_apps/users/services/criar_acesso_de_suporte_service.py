...
def criar_acesso_de_suporte(unidade_do_suporte: Unidade, usuario_do_suporte: User):
    ...
    if not usuario_do_suporte or not isinstance(usuario_do_suporte, get_user_model()):
        ...
        raise CriaAcessoSuporteException(
            'É necessário informar um usuário do suporte pelo parâmetro usuário_do_suporte')

    logger.info(
        f'Acesso de suporte solicitado por {usuario_do_suporte.username} para a unidade {unidade_do_suporte.codigo_eol}')
    ...
    novo_unidade_em_suporte = UnidadeEmSuporte.criar_acesso_suporte_se_nao_existir(
        unidade=unidade_do_suporte, user=usuario_do_suporte)

    return novo_unidade_em_suporte


def criar_acesso_de_suporte_v2(unidade_do_suporte: Unidade, usuario_do_suporte: User):
    logger.info('Criação de acesso de suporte.')
    ...
    if usuario_do_suporte.acessos_de_suporte.filter(unidade__codigo_eol=unidade_do_suporte.codigo_eol).exists():
        logger.info('Usuário já tem acesso à unidade. Acesso de suporte não criado.')
        raise CriaAcessoSuporteException(
            'Você já tem acesso a essa unidade e a unidade não será incluída novamente na lista de unidades vinculadas.')
