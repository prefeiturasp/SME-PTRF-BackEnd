...
def test_criar_usuario_servidor_visao_sme(
    ...
    assert u.username == '9876543'
    assert u.email == 'lukaku@gmail.com'
    assert u.name == 'Lukaku Silva'
    assert u.e_servidor == True
    assert u.visoes.filter(nome='UE').first()
    assert u.visoes.filter(nome='SME').first()
    assert u.groups.filter(name='grupo1').first()
