...
def test_valida_ok_sem_conta_ignorado(validator):
    ctx = make_ctx(
...
def test_valida_ok_sem_data_transacao(validator):
    ctx = make_ctx(
        data_transacao=None,
        rateios=[{"conta_associacao": _conta(date(2020, 1, 1))}],
    )
    result = validator.validate(ctx)
    assert result is ctx
...
def test_valida_ok_sem_data_inicio_e_encerramento(validator):
    ctx = make_ctx(
        data_transacao=date(2020, 3, 10),
        rateios=[{"conta_associacao": _conta(None, None)}],
    )
    result = validator.validate(ctx)
    assert result is ctx
