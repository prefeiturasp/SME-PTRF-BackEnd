...
describe('Validar rotas de acoes da aplicação SigEscola', () => {
	...
	context('Casos de teste para a rota de Get /api/comissoes/', () => {
		...
		it('Validar Get no endpoint /api/comissoes/ retornando lista', () => {
			cy.validar_comissoes('').then((response) => {
				expect(response.status).to.eq(200)
	...
	context('Casos de teste para a rota de Get /api/comissoes/{uuid}/', () => {
		...
		it('Validar Get no endpoint /api/comissoes/{uuid}/ com uuid inexistente', () => {
	...
	context('Casos de teste para a rota de Post /api/comissoes/', () => {
		...
		it('Validar Post no endpoint /api/comissoes/ com nome muito longo', () => {
			var body = { nome: "a".repeat(300) }
			cy.cadastrar_comissoes(body).then((response) => {
				expect(response.status).to.eq(400)
			})
		})

		it('Validar Post no endpoint /api/comissoes/ sem enviar body', () => {
			cy.cadastrar_comissoes({}).then((response) => {
				expect(response.status).to.eq(400)
			})
		})
	...
	context('Casos de teste para a rota de Delete /api/comissoes/{uuid}/', () => {
		...
		it('Validar Delete no endpoint /api/comissoes/{uuid}/ já excluído', () => {
			var body = { nome: "teste automatizado" }
			cy.cadastrar_comissoes(body).then((responseCadastro) => {
				expect(responseCadastro.status).to.eq(201)
				var id = responseCadastro.body.uuid
				cy.excluir_comissoes(id).then((responseExcluir) => {
					expect(responseExcluir.status).to.eq(204)
					cy.excluir_comissoes(id).then((responseExcluirNovamente) => {
						expect(responseExcluirNovamente.status).to.eq(404)
					})
				})
			})
		})
	...
	context('Casos de teste para a rota de Put /api/comissoes/{uuid}/', () => {
		...
		it('Validar Put no endpoint /api/comissoes/{uuid}/ com uuid invalido', () => {
			var body = { nome: "teste automatizado editado" }
			var id = 'fd4'
			cy.alterar_comissoes(body, id).then((response) => {
				expect(response.status).to.eq(404)
			})
		})
	...
	context('Casos de teste para a rota de Patch /api/comissoes/{uuid}/', () => {
		...
		it('Validar Patch no endpoint /api/comissoes/{uuid}/ com uuid invalido', () => {
			var body = { nome: "teste automatizado editado" }
			var id = 'fd4'
			cy.editar_comissoes(body, id).then((response) => {
				expect(response.status).to.eq(404)
			})
		})
