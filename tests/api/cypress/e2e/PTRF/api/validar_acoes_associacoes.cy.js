...
describe('Validar rotas de acoes da aplicação SigEscola', () => {
	...
	context('Casos de teste para a rota de Post api/acoes-associacoes/', () => {
		...
		it('Validar Post no endpoint api/acoes-associacoes/ com acao invalida', () => {
			...
			cy.cadastrar_acoes_associacoes(body).then((response) => {
				expect(response.status).to.eq(400)
				expect(response.body.acao[0]).to.eq(`“${body.acao}” is not a valid UUID.`);
			})
		})
	...
	context('Casos de teste para a rota de delete api/acoes-associacoes/{uuid}/', () => {
	...
	context('Casos de teste para a rota de Put api/acoes-associacoes/{uuid}/', () => {
		...
		it('Validar Put no endpoint api/acoes-associacoes/{uuid}/ com associacao invalida', () => {
			...
			cy.cadastrar_acoes_associacoes(body).then((responseCadastro) => {
				...
				cy.editar_acoes_associacoes(body_alteracao, id).then((response) => {
					expect(response.status).to.eq(400)
					expect(response.body.associacao[0]).to.eq(`“${body_alteracao.associacao}” is not a valid UUID.`);
					cy.excluir_acoes_associacoes(id).then((responseExclusao) => {
						expect(responseExclusao.status).to.eq(204)
					})
				})
		...
		it('Validar Put no endpoint api/acoes-associacoes/{uuid}/ com acao invalida', () => {
			...
			cy.cadastrar_acoes_associacoes(body).then((responseCadastro) => {
				...
				cy.editar_acoes_associacoes(body_alteracao, id).then((response) => {
					expect(response.status).to.eq(400)
					expect(response.body.acao[0]).to.eq(`“${body_alteracao.acao}” is not a valid UUID.`);
					cy.excluir_acoes_associacoes(id).then((responseExclusao) => {
						expect(responseExclusao.status).to.eq(204)
					})
				})
		...
		it('Validar Put no endpoint api/acoes-associacoes/{uuid}/ com Status invalido', () => {
			...
			cy.cadastrar_acoes_associacoes(body).then((responseCadastro) => {
				...
				cy.editar_acoes_associacoes(body_alteracao, id).then((response) => {
					expect(response.status).to.eq(400)
					expect(response.body.status[0]).to.eq(`"${body_alteracao.status}" is not a valid choice.`);
					cy.excluir_acoes_associacoes(id).then((responseExclusao) => {
						expect(responseExclusao.status).to.eq(204)
					})
				})
	...
	context('Casos de teste para a rota de Post api/acoes-associacoes/incluir-lote/', () => {
		it('Validar Post no endpoint api/acoes-associacoes/incluir-lote/ com sucesso', () => {
			...
			cy.cadastrar_acoes_associacoes_incluir_lote(body).then((response) => {
				expect(response.status).to.eq(201)
				expect(response.body.mensagem).to.eq('Unidades vinculadas à ação com sucesso.')
			})
		})
		...
		it('Validar Post no endpoint api/acoes-associacoes/incluir-lote/ com acao_uuid em branco', () => {
			...
			cy.cadastrar_acoes_associacoes_incluir_lote(body).then((response) => {
				...
				expect(response.body.mensagem).to.eq("É necessário enviar a acao_uuid e lista associacoes_uuids.")
			})
		})

	})

	context('Casos de teste para a rota de Post api/acoes-associacoes/excluir-lote/', () => {
		...
		it('Validar Post no endpoint api/acoes-associacoes/excluir-lote/ com valor invalido', () => {
			...
			cy.cadastrar_acoes_associacoes_excluir_lote(body).then((response) => {
				expect(response.body.erro).to.eq("problema_ao_excluir_acoes_associacoes")
				expect(response.body.mensagem).to.eq("badly formed hexadecimal UUID string")
				expect(response.status).to.eq(400)
			})
		})
		...
		it('Validar Post no endpoint api/acoes-associacoes/excluir-lote/ com valor em branco', () => {
			...
			cy.cadastrar_acoes_associacoes_excluir_lote(body).then((response) => {
				expect(response.status).to.eq(400)
				expect(response.body.erro).to.eq("Falta de informações")
				expect(response.body.mensagem).to.eq('É necessário enviar a lista de uuids a serem apagados (lista_uuids).')
			})
		})
	...
	context('Casos de teste para a rota de Get api/acoes-associacoes/', () => {
		it('Validar Get no endpoint api/acoes-associacoes/ com acao uuid valido e status inativo', () => {
			var id = '?acao__uuid=82a4bd9a-5884-4a43-9ae9-4fee0855f9b7&associacao__uuid=&status=INATIVA'
		...
		it('Validar Get no endpoint api/acoes-associacoes/ com acao uuid valido e status ativo', () => {
			var id = '?acao__uuid=82a4bd9a-5884-4a43-9ae9-4fee0855f9b7&associacao__uuid=&status=ATIVA'
			cy.validar_acoes_associacoes(id).then((response) => {
				expect(response.status).to.eq(200)
				expect(response.body).to.exist
			})
		...
		it('Validar Get no endpoint api/acoes-associacoes/ com acao, associacao uuid valido e status ativo', () => {
			var id = '?acao__uuid=82a4bd9a-5884-4a43-9ae9-4fee0855f9b7&associacao__uuid=5aa26087-0163-4b7a-b621-8eb1452e909b&status=ATIVA'
			cy.validar_acoes_associacoes(id).then((response) => {
				expect(response.status).to.eq(200)
				expect(response.body.results[0].acao.uuid).to.eq('82a4bd9a-5884-4a43-9ae9-4fee0855f9b7')
				...
				expect(response.body.results[0].associacao.status_valores_reprogramados).to.exist
		...
		it('Validar Get no endpoint api/acoes-associacoes/ com acao, associacao uuid valido e status Inativo', () => {
			var id = '?acao__uuid=82a4bd9a-5884-4a43-9ae9-4fee0855f9b7&associacao__uuid=5aa26087-0163-4b7a-b621-8eb1452e909b&status=INATIVA'
			cy.validar_acoes_associacoes(id).then((response) => {
				expect(response.status).to.eq(200)
				expect(response.body).to.exist
			})
		...
		it('Validar Get no endpoint api/acoes-associacoes/ com dados invalidos', () => {
			...
			cy.validar_acoes_associacoes(id).then((response) => {
				...
				expect(response.body.status[0]).to.eq("Select a valid choice. fdsfs is not one of the available choices.")
			})
		})

	})

	context('Casos de teste para a rota de Get api/acoes-associacoes/uuid', () => {
		it('Validar Get no endpoint api/acoes-associacoes/uuid com sucesso', () => {
			...
			cy.validar_acoes_associacoes(id).then((response) => {
				...
				expect(response.body.associacao.status_valores_reprogramados).to.exist
