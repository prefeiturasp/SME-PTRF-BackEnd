...
describe('Validar rotas de acoes da aplicação SigEscola', () => {
	...
	context('Casos de teste para a rota de Get /api/acoes-pdde/', () => {
		...
		it('Validar Get no endpoint /api/acoes-pdde/ com sucesso e com todos parametros preenchidos paramentros', () => {
			var id = '?aceita_capital=true&aceita_custeio=false&aceita_livre_aplicacao=true&page=1&page_size=10'
			cy.validar_acoes_pdde(id).then((response) => {
				expect(response.status).to.eq(200)
				expect(response.body.results).to.exist
				expect(response.body.page).to.eq(1)
		...
		it('Validar Get no endpoint /api/acoes-pdde/{uuid}/ com sucesso', () => {
			...
			cy.validar_acoes_pdde(id).then((response) => {
				...
				cy.validar_acoes_pdde(id).then((response) => {
					...
					expect(response.body.categoria_objeto).to.exist
				})
			})
		})
	...
	context('Casos de teste para a rota de Post /api/acoes-pdde/', () => {
		it('Validar Post no endpoint /api/acoes-pdde/ com sucesso', () => {
			...
			cy.cadastrar_acoes_pdde(body).then((response) => {
				...
				expect(response.body.aceita_livre_aplicacao).to.eq(body.aceita_livre_aplicacao)
				expect(response.body.categoria).to.eq(body.categoria)
				expect(response.body.id).to.exist
		...
		it('Validar Post no endpoint /api/acoes-pdde/ com nome duplicado', () => {
			...
			cy.cadastrar_acoes_pdde(body).then((responseCadastro) => {
				...
				cy.cadastrar_acoes_pdde(body).then((response) => {
					...
					expect(response.body.detail).to.eq("Erro ao criar Ação PDDE. Já existe uma Ação PDDE cadastrada com este nome e categoria.")
		...
		it('Validar Post no endpoint /api/acoes-pdde/ com nome em branco', () => {
			...
			cy.cadastrar_acoes_pdde(body).then((response) => {
				expect(response.status).to.eq(400)
				expect(response.body.nome).to.eq("Nome da ação PDDE não foi informado.")
			})
		})
		...
		it('Validar Post no endpoint /api/acoes-pdde/ nome com mais de 160 posicoes', () => {
			...
			cy.cadastrar_acoes_pdde(body).then((response) => {
				expect(response.status).to.eq(400)
				expect(response.body.nome[0]).to.eq("Ensure this field has no more than 160 characters.")
			})
		})

		it('Validar Post no endpoint /api/acoes-pdde/ com categoria zerada', () => {
			var body = {
				nome: "teste automatizado",
				categoria: 0,
				aceita_capital: true,
			...
			cy.cadastrar_acoes_pdde(body).then((response) => {
				expect(response.status).to.eq(400)
				expect(response.body.categoria).to.eq("A Categoria PDDE não foi informada.")
			})
		})

		it('Validar Post no endpoint /api/acoes-pdde/ com categoria em branco', () => {
			var body = {
				nome: "teste automatizado",
				categoria: '',
				aceita_capital: true,
		...
		it('Validar Post no endpoint /api/acoes-pdde/ com categoria null', () => {
			var body = {
				nome: "teste automatizado",
				categoria: null,
				aceita_capital: true,
		...
		it('Validar Post no endpoint /api/acoes-pdde/ com aceita custeio com valor invalido', () => {
			...
			cy.cadastrar_acoes_pdde(body).then((response) => {
				expect(response.status).to.eq(400)
				expect(response.body.aceita_livre_aplicacao[0]).to.eq("Must be a valid boolean.")
			})
		})
	...
	context('Casos de teste para a rota de Delete /api/acoes-pdde/{uuid}/', () => {
	...
	context('Casos de teste para a rota de Put /api/acoes-pdde/{uuid}/', () => {
		it('Validar Put no endpoint /api/acoes-pdde/{uuid}/ com sucesso', () => {
			...
			cy.cadastrar_acoes_pdde(body).then((response) => {
				...
				cy.alterar_acoes_pdde(body, id).then((responseAlterar) => {
					...
					expect(responseAlterar.body.aceita_capital).to.eq(body.aceita_capital)	
					expect(responseAlterar.body.aceita_custeio).to.eq(body.aceita_custeio)
					expect(responseAlterar.body.aceita_livre_aplicacao).to.eq(body.aceita_livre_aplicacao)
					expect(responseAlterar.body.categoria).to.eq(body.categoria)
		...
		it('Validar Put no endpoint /api/acoes-pdde/{uuid}/ com nome em branco', () => {
			...
			cy.cadastrar_acoes_pdde(body).then((response) => {
				...
				cy.alterar_acoes_pdde(body, id).then((responseAlterar) => {
					...
					expect(responseAlterar.body.nome).to.eq("Nome da ação PDDE não foi informado.")
				cy.excluir_acoes_pdde(id).then((responseExcluir) => {
					expect(responseExcluir.status).to.eq(204)
				})
			})
		...
		it('Validar Put no endpoint /api/acoes-pdde/{uuid}/ com nome com mais de 160 posições', () => {
			...
			cy.cadastrar_acoes_pdde(body).then((response) => {
				...
				cy.alterar_acoes_pdde(body, id).then((responseAlterar) => {
					expect(responseAlterar.status).to.eq(400)
					expect(responseAlterar.body.nome[0]).to.eq("Ensure this field has no more than 160 characters.")
				cy.excluir_acoes_pdde(id).then((responseExcluir) => {
					expect(responseExcluir.status).to.eq(204)
				})
			})
		...
		it('Validar Put no endpoint /api/acoes-pdde/{uuid}/ com categoria em branco', () => {
			var body = {
				nome: "teste automatizado",
				categoria: 1,
				aceita_capital: true,
			...
			cy.cadastrar_acoes_pdde(body).then((response) => {
				...
				cy.alterar_acoes_pdde(body, id).then((responseAlterar) => {
					expect(responseAlterar.status).to.eq(400)
					expect(responseAlterar.body.categoria).to.eq("A Categoria PDDE não foi informada.")
				cy.excluir_acoes_pdde(id).then((responseExcluir) => {
					expect(responseExcluir.status).to.eq(204)
				})
			})
		...
		it('Validar Put no endpoint /api/acoes-pdde/{uuid}/ com categoria zerada', () => {
			var body = {
				nome: "teste automatizado",
				categoria: 1,
				aceita_capital: true,
		...
		it('Validar Put no endpoint /api/acoes-pdde/{uuid}/ com aceita capital em branco', () => {
			...
			cy.cadastrar_acoes_pdde(body).then((response) => {
				...
				cy.alterar_acoes_pdde(body, id).then((responseAlterar) => {
					expect(responseAlterar.status).to.eq(400)
					expect(responseAlterar.body.aceita_capital[0]).to.eq("Must be a valid boolean.")
				cy.excluir_acoes_pdde(id).then((responseExcluir) => {
					expect(responseExcluir.status).to.eq(204)
				})
			})
		...
		it('Validar Put no endpoint /api/acoes-pdde/{uuid}/ com aceita custeio inválido', () => {
			...
			cy.cadastrar_acoes_pdde(body).then((response) => {
				...
				cy.alterar_acoes_pdde(body, id).then((responseAlterar) => {
					expect(responseAlterar.status).to.eq(400)
					expect(responseAlterar.body.aceita_custeio[0]).to.eq("Must be a valid boolean.")
				cy.excluir_acoes_pdde(id).then((responseExcluir) => {
					expect(responseExcluir.status).to.eq(204)
				})
			})
		...
		it('Validar Put no endpoint /api/acoes-pdde/{uuid}/ com aceita livre aplicacao inválido', () => {
			...
			cy.cadastrar_acoes_pdde(body).then((response) => {
				...
				cy.alterar_acoes_pdde(body, id).then((responseAlterar) => {
					expect(responseAlterar.status).to.eq(400)
					expect(responseAlterar.body.aceita_livre_aplicacao[0]).to.eq("Must be a valid boolean.")
				cy.excluir_acoes_pdde(id).then((responseExcluir) => {
					expect(responseExcluir.status).to.eq(204)
				})
			})
	...
	context('Casos de teste para a rota de Patch /api/acoes-pdde/{uuid}/', () => {
		...
		it('Validar Patch no endpoint /api/acoes-pdde/{uuid}/ com categoria em branco', () => {
			var body = {
				nome: "teste automatizado",
				categoria: 1,
				aceita_capital: true,
		...
		it('Validar Patch no endpoint /api/acoes-pdde/{uuid}/ com categoria zerada', () => {
			var body = {
				nome: "teste automatizado",
				categoria: 1,
				aceita_capital: true,
