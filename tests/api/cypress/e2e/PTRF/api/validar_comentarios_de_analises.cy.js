...
describe("Validar rotas de acoes da aplicação SigEscola", () => {
  ...
  context(
    "Casos de teste para a rota de Get /api/comentarios-de-analises/",
    () => {
      ...
      it.only("Validar Get no endpoint /api/comentarios-de-analises/{uuid}/ com sucesso", () => {
        ...
        cy.validar_comentarios_de_analises(id).then((response) => {
          ...
          cy.validar_comentarios_de_analises(id).then((response) => {
            ...
            expect(response.body.associacao);
            expect(response.body.comentario).to.exist;
            expect(response.body.notificado).to.exist;
            expect(response.body.notificado_em);
            expect(response.body.ordem);
            expect(response.body.periodo);
            expect(response.body.prestacao_conta);
          });
        });
      });
  ...
  context(
    "Casos de teste para a rota de Post /api/comentarios-de-analises/",
    () => {
      ...
      it("Validar Post no endpoint /api/comentarios-de-analises/ com prestação de conta invalido", () => {
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          ...
          expect(response.body.prestacao_conta[0]).to.eq(
            `“${body.prestacao_conta}” is not a valid UUID.`
          );
        });
      });
      ...
      it("Validar Post no endpoint /api/comentarios-de-analises/ com prestação de conta em branco", () => {
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          ...
          expect(response.body.detail).to.eq(
            "É necessário enviar a prestação de contas ou associação e período."
          );
        });
      });
      ...
      it("Validar Post no endpoint /api/comentarios-de-analises/ com periodo invalido", () => {
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          expect(response.status).to.eq(400);
          expect(response.body.periodo[0]).to.eq(
            `“${body.periodo}” is not a valid UUID.`
          );
        });
      });
      ...
      it("Validar Post no endpoint /api/comentarios-de-analises/ com notificado_em em branco", () => {
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          expect(response.status).to.eq(400);
          expect(response.body.notificado_em[0]).to.eq(
  ...
  context(
    "Casos de teste para a rota de Delete /api/comentarios-de-analises/{uuid}/",
    () => {
  ...
  context(
    "Casos de teste para a rota de Put /api/comentarios-de-analises/{uuid}/",
    () => {
      it("Validar Put no endpoint /api/comentarios-de-analises/ com sucesso", () => {
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          ...
          cy.alterar_comentarios_de_analises(body, id).then(
            (responseAlterar) => {
              ...
              expect(responseAlterar.body.prestacao_conta).to.eq(
                body.prestacao_conta
              );
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          ...
          cy.alterar_comentarios_de_analises(body, id).then(
            (responseAlterar) => {
              ...
              expect(responseAlterar.body.prestacao_conta[0]).to.eq(
                `“${body.prestacao_conta}” is not a valid UUID.`
              );
              cy.excluir_comentarios_de_analises(id).then((responseExcluir) => {
                expect(responseExcluir.status).to.eq(204);
      ...
      it("Validar Put no endpoint /api/comentarios-de-analises/ com prestação de conta em branco", () => {
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          ...
          cy.alterar_comentarios_de_analises(body, id).then(
            (responseAlterar) => {
              expect(responseAlterar.status).to.eq(400);
              expect(responseAlterar.body.detail).to.eq(
                "É necessário enviar a prestação de contas ou associação e período."
              );
              cy.excluir_comentarios_de_analises(id).then((responseExcluir) => {
                expect(responseExcluir.status).to.eq(204);
      ...
      it("Validar Put no endpoint /api/comentarios-de-analises/ com associacao invalida", () => {
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          ...
          cy.alterar_comentarios_de_analises(body, id).then(
            (responseAlterar) => {
              expect(responseAlterar.status).to.eq(400);
              expect(responseAlterar.body.associacao[0]).to.eq(
                `“${body.associacao}” is not a valid UUID.`
              );
              cy.excluir_comentarios_de_analises(id).then((responseExcluir) => {
                expect(responseExcluir.status).to.eq(204);
      ...
      it("Validar Put no endpoint /api/comentarios-de-analises/ com periodo invalido", () => {
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          ...
          cy.alterar_comentarios_de_analises(body, id).then(
            (responseAlterar) => {
              expect(responseAlterar.status).to.eq(400);
              expect(responseAlterar.body.periodo[0]).to.eq(
                `“${body.periodo}” is not a valid UUID.`
              );
              cy.excluir_comentarios_de_analises(id).then((responseExcluir) => {
                expect(responseExcluir.status).to.eq(204);
      ...
      it("Validar Put no endpoint /api/comentarios-de-analises/ com ordem em branco", () => {
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          ...
          cy.alterar_comentarios_de_analises(body, id).then(
            (responseAlterar) => {
              expect(responseAlterar.status).to.eq(400);
              expect(responseAlterar.body.ordem[0]).to.eq(
      ...
      it("Validar Put no endpoint /api/comentarios-de-analises/ com notificado invalido", () => {
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          ...
          cy.alterar_comentarios_de_analises(body, id).then(
            (responseAlterar) => {
              expect(responseAlterar.status).to.eq(400);
              expect(responseAlterar.body.notificado[0]).to.eq(
      ...
      it("Validar Put no endpoint /api/comentarios-de-analises/ com notificado_em em branco", () => {
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          ...
          cy.alterar_comentarios_de_analises(body, id).then(
            (responseAlterar) => {
              expect(responseAlterar.status).to.eq(400);
              expect(responseAlterar.body.notificado_em[0]).to.eq(
                "Date has wrong format. Use one of these formats instead: YYYY-MM-DD."
              );
              cy.excluir_comentarios_de_analises(id).then((responseExcluir) => {
                expect(responseExcluir.status).to.eq(204);
  ...
  context(
    "Casos de teste para a rota de Patch /api/comentarios-de-analises/{uuid}/",
    () => {
      it("Validar Patch no endpoint /api/comentarios-de-analises/ com sucesso", () => {
        ...
        cy.cadastrar_comentarios_de_analises(body).then((response) => {
          ...
          cy.editar_comentarios_de_analises(body, id).then(
            (responseAlterar) => {
              expect(responseAlterar.status).to.eq(200);
              expect(responseAlterar.body.comentario).to.eq(body.comentario);
              expect(responseAlterar.body.associacao).to.null;
